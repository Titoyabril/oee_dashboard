...
class CSS2DDashboard {
    ...
    createMachineStatusCircles(bottomGroup) {
        ...
        machineStatuses.forEach((machine, index) => {
            ...
            // Create ultra-realistic glass cube with high-resolution geometry
            const cubeSize = circleRadius * 1.2;
            const cubeGeometry = new THREE.BoxGeometry(cubeSize, cubeSize, cubeSize, 32, 32, 32);
            let glassMaterial;
            
            if (machine.status === 'fault') {
                // Ultra-realistic red glass cube with advanced properties
                glassMaterial = new THREE.MeshPhysicalMaterial({
                    ...
                    reflectivity: 1.0,
                    attenuationColor: new THREE.Color(1.0, 0.1, 0.1),
                ...
                // Ultra-realistic clear glass cube with advanced properties
                glassMaterial = new THREE.MeshPhysicalMaterial({
                    transmission: 1.0,
                    opacity: 1.0,
                    thickness: 2.0,
            ...
            statusDot.style.background = machine.status === 'fault' ? '#ff4444' : '#00ff00';
            statusDot.style.borderRadius = '50%';
            statusDot.style.boxShadow = `0 0 10px ${machine.status === 'fault' ? '#ff4444' : '#00ff00'}`;
            if (machine.status === 'fault') {
