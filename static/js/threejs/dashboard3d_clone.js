/**
 * Three.js Dashboard Clone - Holographic Compass Version
 * Bottom section with gears and holographic spinning compass
 */
...
class HolographicCompassDashboard {
    constructor() {
        ...
        this.kpiData = [
            { label: 'QUALITY', value: 98, color: 0x00ff88 },
            { label: 'PERFORMANCE', value: 88, color: 0x00d4ff },
            { label: 'AVAILABILITY', value: 95, color: 0xffaa00 },
            { label: 'OEE', value: 82, color: 0xff00ff }
        ];
        this.currentKPIIndex = 0;
        this.isSpinning = true;
        this.spinSpeed = 0.02;
        this.displayDuration = 3000; // 3 seconds per KPI
        
        // Color schemes for the model - updated for futuristic cyan hologram
        this.colorSchemes = {
            classic: { primary: 0x00ffff, secondary: 0x00ccff }, // Cyan variants
            ironSpider: { primary: 0x00ffff, secondary: 0x0099ff }, // Cyan to blue
            symbiote: { primary: 0x00ffff, secondary: 0x00ffcc }, // Cyan to teal
            stealth: { primary: 0x00ffff, secondary: 0x66ffff }, // Cyan shades
            spiderVerse: { primary: 0x00ffff, secondary: 0x00ff99 }, // Cyan to aqua
            holographic: { primary: 0x00ffff, secondary: 0x00e6ff } // Pure cyan tones
        };
        this.currentColorScheme = 'holographic'; // Default scheme
        
        // Animation settings
        this.animations = {
            rotation: { enabled: true, speed: 0.01 },
            floating: { enabled: true, amplitude: 10, speed: 0.002 },
            tilting: { enabled: true, amplitude: 0.1, speed: 0.003 },
            scaling: { enabled: false, amplitude: 0.1, speed: 0.001 },
            pose: {
                enabled: true,
                armSwing: { amplitude: 0.3, speed: 0.004 },
                legMovement: { amplitude: 0.2, speed: 0.003 },
                headBob: { amplitude: 0.1, speed: 0.005 }
            }
        };
        this.basePosition = { x: 0, y: -25, z: 0 };
        this.spiderManModel = null;
    ...
    init() {
        ...
        // Configure modern lighting for Three.js r155+
        ...
        // Create holographic compass in middle section
        this.createHolographicCompass();
    ...
    createHolographicSpiderMan() {
        // Load the actual Spider-Man 3D model
        this.spiderManGroup = new THREE.Group();
        this.spiderManGroup.position.set(0, 50, 0);
        
        const loader = new GLTFLoader();
        
        console.log('=== UPDATED - Loading Looking Glass Hologram ===');
        console.log('Model path: /static/models/looking_glass_hologram/scene.gltf');
        console.log('Timestamp:', Date.now()); // Force cache break
        
        loader.load(
            '/static/models/looking_glass_hologram/scene.gltf',
            (gltf) => {
                console.log('Looking Glass Hologram model loaded successfully!');
                
                const model = gltf.scene;
                
                // Scale up model to cover middle and top sections
                model.scale.setScalar(120); // Scaled down by 20% (150 * 0.8 = 120)
                model.position.set(0, -120, 0); // Brought down by another 20
                
                // Store the model and base position for animations
                this.spiderManModel = model;
                this.spiderManBaseY = -120; // Updated base position
                
                // OPTION 2: Store positions for reset method
                console.log('=== OPTION 2: STORING MESH POSITIONS FOR RESET METHOD ===');
                this.meshData = [];
                
                model.traverse((child) => {
                    if (child.isMesh) {
                        console.log(`Storing data for mesh: ${child.name || 'unnamed'}`);
                        
                        // Check if this mesh might be text
                        if (child.geometry && child.geometry.type === 'BufferGeometry') {
                            const vertexCount = child.geometry.attributes.position?.count || 0;
                            console.log(`  - Geometry type: ${child.geometry.type}, Vertices: ${vertexCount}`);
                            
                            // Only hide meshes that are clearly text (Objects with numbered names are likely text)
                            if (child.name.match(/^Object_\d+$/) || 
                                child.name.toLowerCase().includes('text') || 
                                child.name.toLowerCase().includes('font')) {
                                console.log(`  - TEXT MESH HIDDEN: ${child.name}`);
                                child.visible = false;
                            } else {
                        ...
                        const meshInfo = {
                            ...
                            rotationAccumulator: 0 // Track rotation amount
                        };
                        
                        // Get world transforms
                        child.getWorldPosition(meshInfo.worldPosition);
                        child.getWorldScale(meshInfo.worldScale);
                        
                        this.meshData.push(meshInfo);
                        
                        console.log(`  - Original pos:`, meshInfo.originalPosition.toArray());
                        console.log(`  - World pos:`, meshInfo.worldPosition.toArray());
                    }
                });
                
                console.log(`Stored data for ${this.meshData.length} meshes`);
                
                // Log model structure to understand donut components
                console.log('Model structure analysis:');
                this.analyzeModelStructure(model);
                
                // Apply holographic materials to all meshes
                this.applyColorSchemeToModel(model);
                
                this.spiderManGroup.add(model);
                
                // REMOVED: All extra elements - just showing the model
                // this.createWebEffects();
                // this.createHolographicDisplay();
                // this.compassGroup = this.spiderManGroup;
                // if (this.kpiDisplay) {
                //     this.spiderManGroup.add(this.kpiDisplay);
                // }
                
                this.scene.add(this.spiderManGroup);
                
                // REMOVED: Display cycle that creates text
                // this.startSpiderManCycle();
                
                console.log('Spider-Man holographic display ready!');
            },
            (progress) => {
                console.log('Loading progress:', (progress.loaded / progress.total * 100) + '%');
            },
            (error) => {
                console.error('Error loading Looking Glass Hologram model:', error);
                console.log('Falling back to procedural model...');
                
                // Fallback to procedural model if loading fails
                this.createProceduralDonutModel();
            }
        );
    }
    
    createProceduralDonutModel() {
        console.log('Creating procedural donut model...');
        
        // Create container group
        const model = new THREE.Group();
        model.name = 'proceduralDonutModel';
        ...
        // Create multiple donuts (torus) at different positions
        for (let i = 0; i < 5; i++) {
            const donutGeometry = new THREE.TorusGeometry(15, 5, 8, 16);
            const donut = new THREE.Mesh(donutGeometry, i % 2 === 0 ? material1 : material2);
            
            // Position donuts in a pattern
        ...
        // Store model data for animation
        this.spiderManModel = model;
        this.spiderManBaseY = -25;
        this.meshData = [];
        
        // Store mesh data for rotation
        model.traverse((child) => {
            if (child.isMesh) {
                const meshInfo = {
                    mesh: child,
                ...
                this.meshData.push(meshInfo);
                console.log(`Added mesh to animation: ${child.name}`);
            }
        });
        ...
        // Start the animation and cycle
        console.log('Procedural model ready for animation!');
        
        console.log(`Created procedural model with ${this.meshData.length} meshes`);
    }
    
    applyColorSchemeToModel(model) {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                const useSecondary = meshIndex % 2 === 1;
                const colorToUse = useSecondary ? colors.secondary : colors.primary;
                
                // Futuristic cyan holographic material
        ...
        console.log(`Applied ${this.currentColorScheme} color scheme to Spider-Man model`);
    }
    ...
    changeColorScheme(schemeName) {
        if (this.colorSchemes[schemeName]) {
            ...
            // Re-apply colors to the existing model
            if (this.spiderManGroup) {
                this.spiderManGroup.traverse((child) => {
                    if (child.isGroup || child.isObject3D) {
                        this.applyColorSchemeToModel(child);
                    }
                });
            }
            
            console.log(`Changed to ${schemeName} color scheme`);
        } else {
            console.log(`Color scheme '${schemeName}' not found. Available schemes:`, Object.keys(this.colorSchemes));
        }
    }
    ...
    cycleColorScheme() {
        ...
        this.changeColorScheme(schemes[nextIndex]);
    }
    
    createProceduralSpiderMan() {
        // Fallback procedural Spider-Man (simplified version of original)
        const spiderMaterial = new THREE.MeshPhysicalMaterial({
            color: 0xff0000,
            metalness: 0.8,
            roughness: 0.2,
            transparent: true,
        ...
        head.position.y = 15;
        this.spiderManGroup.add(head);
        
        // Add web effects around procedural Spider-Man
        this.createWebEffects();
        
        // Create holographic display before starting cycle
        this.createHolographicDisplay();
        
        // Store for backward compatibility
        this.compassGroup = this.spiderManGroup;
        
        // Add the KPI display to the group now that it exists
        if (this.kpiDisplay) {
            this.spiderManGroup.add(this.kpiDisplay);
        }
    ...
    createSpiderManBody(redMaterial, blueMaterial) {
        // Head
        ...
        leftEye.position.set(-5, 65, 12);
        leftEye.scale.set(1.2, 1.5, 0.5);
        
        const rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
        rightEye.position.set(5, 65, 12);
        rightEye.scale.set(1.2, 1.5, 0.5);
        
        this.spiderManGroup.add(leftEye);
        this.spiderManGroup.add(rightEye);
        
        // Torso
        const torsoGeometry = new THREE.CylinderGeometry(12, 15, 35, 8);
        const torso = new THREE.Mesh(torsoGeometry, redMaterial);
        torso.position.y = 30;
        this.spiderManGroup.add(torso);
        
        // Spider emblem
        const spiderGeometry = new THREE.OctahedronGeometry(5, 0);
        const spiderEmblem = new THREE.Mesh(spiderGeometry, 
            new THREE.MeshBasicMaterial({
                color: 0x000000,
                emissive: 0x111111
            })
        );
        spiderEmblem.position.set(0, 35, 10);
        spiderEmblem.rotation.z = Math.PI / 4;
        this.spiderManGroup.add(spiderEmblem);
        
        // Arms
        const armGeometry = new THREE.CylinderGeometry(4, 4, 30, 8);
        ...
        this.spiderManGroup.add(leftArm);
        this.spiderManGroup.add(rightArm);
        
        // Legs  
        const legGeometry = new THREE.CylinderGeometry(5, 4, 35, 8);
        ...
        this.spiderManGroup.add(leftLeg);
        this.spiderManGroup.add(rightLeg);
        
        // Web shooters
        const webShooterGeometry = new THREE.BoxGeometry(6, 3, 3);
        const webShooterMaterial = new THREE.MeshBasicMaterial({
            color: 0xcccccc,
            emissive: 0x00ffff,
            emissiveIntensity: 0.5
        });
        
        const leftWebShooter = new THREE.Mesh(webShooterGeometry, webShooterMaterial);
        leftWebShooter.position.set(-20, 25, 0);
        
        const rightWebShooter = new THREE.Mesh(webShooterGeometry, webShooterMaterial);
        rightWebShooter.position.set(20, 25, 0);
        
        this.spiderManGroup.add(leftWebShooter);
        this.spiderManGroup.add(rightWebShooter);
    }
    
    createWebEffects() {
        // Create animated web lines around Spider-Man
        ...
        for (let i = 0; i < 5; i++) {
            ...
            webLine.userData.isWeb = true;
            webLine.userData.phase = i * 0.5;
            this.spiderManGroup.add(webLine);
        }
    }
    
    createHolographicCompass() {
        // Redirect to Spider-Man creation
        this.createHolographicSpiderMan();
        return;
        
        // Original compass code below (kept for reference)
        this.compassGroup = new THREE.Group();
        this.compassGroup.position.set(0, 50, 0);
        
        // Create compass ring structure
        const outerRadius = 150;
        const innerRadius = 120;
        const segments = 64;
        
        // Outer ring
        const outerRingGeometry = new THREE.TorusGeometry(outerRadius, 2, 8, segments);
        ...
        // Inner ring
        const innerRingGeometry = new THREE.TorusGeometry(innerRadius, 1.5, 8, segments);
        ...
        this.createCompassMarkers();
        
        // Central holographic display area
        this.createHolographicDisplay();
        
        // Add floating particles around compass
        this.createCompassParticles();
        
        this.scene.add(this.compassGroup);
        
        // Start the spinning and display cycle
        this.startCompassCycle();
    }
    ...
    createCompassMarkers() {
        ...
        directions.forEach((dir, index) => {
            ...
            // Create holographic text marker
            const markerDiv = document.createElement('div');
            markerDiv.className = 'compass-marker';
            markerDiv.textContent = dir;
            ...
            markerLabel.position.set(x, 0, z);
            this.compassGroup.add(markerLabel);
        });
    }
    
    createHolographicDisplay() {
        // Container for holographic KPI display
        this.kpiDisplayDiv = document.createElement('div');
        this.kpiDisplayDiv.className = 'holographic-kpi';
        this.kpiDisplayDiv.style.cssText = `
            font-family: 'Orbitron', monospace;
            text-align: center;
            pointer-events: none;
            opacity: 0;
        ...
        this.kpiLabel = document.createElement('div');
        this.kpiLabel.style.cssText = `
            font-size: 18px;
            color: #00ffff;
        ...
        this.kpiValue = document.createElement('div');
        this.kpiValue.style.cssText = `
            font-size: 72px;
            font-weight: bold;
            line-height: 1;
        `;
        
        this.kpiPercent = document.createElement('span');
        this.kpiPercent.style.cssText = `
            font-size: 36px;
            margin-left: 5px;
        `;
        
        this.kpiValue.appendChild(this.kpiPercent);
        this.kpiDisplayDiv.appendChild(this.kpiLabel);
        this.kpiDisplayDiv.appendChild(this.kpiValue);
        
        const kpiDisplay = new CSS2DObject(this.kpiDisplayDiv);
        kpiDisplay.position.set(0, 0, 0);
        
        // Store the display for later addition to the group
        this.kpiDisplay = kpiDisplay;
    }
    
    createCompassParticles() {
        ...
        for (let i = 0; i < innerParticleCount; i++) {
            ...
            innerPositions[i * 3 + 1] = height;
            innerPositions[i * 3 + 2] = Math.sin(angle) * radius;
            
            // Gradient colors from cyan to purple
            const t = i / innerParticleCount;
            innerColors[i * 3] = t;
            innerColors[i * 3 + 1] = 1 - t * 0.5;
            innerColors[i * 3 + 2] = 1;
            
            innerSizes[i] = Math.random() * 3 + 1;
        }
        ...
        this.innerParticles = new THREE.Points(innerGeometry, innerMaterial);
        this.compassGroup.add(this.innerParticles);
        
        // Outer floating particles
        const outerParticleCount = 150;
        ...
        for (let i = 0; i < outerParticleCount; i++) {
            ...
            outerPositions[i * 3 + 1] = height;
            outerPositions[i * 3 + 2] = Math.sin(angle) * radius;
            
            // Holographic blue-green colors
            outerColors[i * 3] = 0;
            outerColors[i * 3 + 1] = 0.8 + Math.random() * 0.2;
            outerColors[i * 3 + 2] = 1;
        }
        ...
        this.outerParticles = new THREE.Points(outerGeometry, outerMaterial);
        this.compassGroup.add(this.outerParticles);
        
        // Store for animation
        this.compassParticles = this.innerParticles; // For backward compatibility
    }
    
    // Post-processing removed due to CORS issues with unpkg
    // Keeping core holographic effects using built-in Three.js features
    
    startSpiderManCycle() {
        // Override KPI data with Spider-Man themed metrics
        this.kpiData = [
            { label: 'WEB STRENGTH', value: 98, color: 0xff0000 },
            { label: 'AGILITY', value: 95, color: 0x0066ff },
            { label: 'SPIDER-SENSE', value: 92, color: 0xffaa00 },
            { label: 'HERO RATING', value: 100, color: 0xff00ff }
        ];
        
        const cycleKPI = () => {
            // Fade out current display
            this.kpiDisplayDiv.style.opacity = '0';
            
            setTimeout(() => {
                // Update to next KPI
                const kpi = this.kpiData[this.currentKPIIndex];
                this.kpiLabel.textContent = kpi.label;
                this.kpiValue.textContent = kpi.value;
                this.kpiPercent.textContent = '%';
                this.kpiValue.style.color = `#${kpi.color.toString(16).padStart(6, '0')}`;
                this.kpiValue.style.textShadow = `
                    0 0 20px #${kpi.color.toString(16).padStart(6, '0')},
                    0 0 40px #${kpi.color.toString(16).padStart(6, '0')},
                    0 0 60px #${kpi.color.toString(16).padStart(6, '0')}
                `;
                
                // Stop spinning briefly
                this.isSpinning = false;
                
                // Trigger web-slinging animation
                if (this.spiderManGroup) {
                    this.spiderManGroup.children.forEach(child => {
                        if (child.userData.isWeb) {
                ...
                // Fade in new display
                this.kpiDisplayDiv.style.opacity = '1';
                
                // Resume spinning after display time
                setTimeout(() => {
                    this.kpiDisplayDiv.style.opacity = '0';
                    ...
                    setTimeout(() => {
                        this.isSpinning = true;
                        this.currentKPIIndex = (this.currentKPIIndex + 1) % this.kpiData.length;
                        cycleKPI();
                    }, 500);
    ...
    triggerAlertState(severity = 'high') {
        ...
        if (!activeFaultCard) {
            console.log('Active fault card not found, skipping alert');
            return;
        }
        
        const warningIndicator = activeFaultCard.querySelector('.warning-indicator');
        if (!warningIndicator) {
            console.log('Warning indicator not found, skipping alert');
            return;
        }
    ...
    setupColorControls() {
        ...
        console.log('Color: Press C to cycle through color schemes (1-6 for specific schemes)');
        console.log('Animation: R=rotation, F=floating, T=tilting, P=donut spinning, S=scaling');
        console.log('Available schemes:', Object.keys(this.colorSchemes));
        ...
        window.addEventListener('keydown', (event) => {
            switch(event.key.toLowerCase()) {
                case 'c':
                    ...
                    break;
                case '1':
                    this.changeColorScheme('classic');
                    break;
                case '2':
                    this.changeColorScheme('ironSpider');
                    break;
                case '3':
                    this.changeColorScheme('symbiote');
                    break;
                case '4':
                    this.changeColorScheme('stealth');
                    break;
                case '5':
                    this.changeColorScheme('spiderVerse');
                    break;
                case '6':
                    this.changeColorScheme('holographic');
                    break;
                case 'r':
                    this.toggleAnimation('rotation');
                    break;
                case 'f':
                    this.toggleAnimation('floating');
                    break;
                case 't':
                    this.toggleAnimation('tilting');
                    break;
                case 'p':
                    this.toggleAnimation('pose');
                    break;
                case 's':
                    this.toggleAnimation('scaling');
                    break;
            }
        });
    }
    ...
    toggleAnimation(animationType) {
        if (this.animations.hasOwnProperty(animationType)) {
            this.animations[animationType].enabled = !this.animations[animationType].enabled;
            console.log(`Animation ${animationType}: ${this.animations[animationType].enabled ? 'ON' : 'OFF'}`);
        } else {
    ...
    analyzeModelStructure(model) {
        ...
        // First, let's see what the entire model structure looks like
        console.log('=== MODEL STRUCTURE ANALYSIS ===');
        ...
        console.log(`Found ${meshesToAnimate.length} meshes to animate`);
        
        // Store for animation
        this.animatableMeshes = meshesToAnimate;
    }
    ...
    animate() {
        ...
        // Animate Spider-Man or compass spinning
        if (this.compassGroup && this.isSpinning) {
            this.compassGroup.rotation.y += this.spinSpeed;
        }
        
        // Animate compass particles with enhanced effects
        const time = Date.now() * 0.001;
        
        // Animate Spider-Man model
        if (this.spiderManModel) {
            // Rotation animation - rotate whole model
            if (this.animations.rotation.enabled) {
                this.spiderManModel.rotation.y += this.animations.rotation.speed;
            }
            
            // Floating animation
            if (this.animations.floating.enabled) {
                this.spiderManModel.position.y = this.spiderManBaseY + 
                    Math.sin(time * this.animations.floating.speed) * this.animations.floating.amplitude;
            }
            
            // Tilting animation
            if (this.animations.tilting.enabled) {
                this.spiderManModel.rotation.x = Math.sin(time * this.animations.tilting.speed) * this.animations.tilting.amplitude;
                this.spiderManModel.rotation.z = Math.cos(time * this.animations.tilting.speed * 0.7) * this.animations.tilting.amplitude * 0.5;
            }
            
            // Scaling animation (breathing effect)
            if (this.animations.scaling.enabled) {
                const scale = 1 + Math.sin(time * this.animations.scaling.speed) * this.animations.scaling.amplitude;
                this.spiderManModel.scale.setScalar(scale);
            }
        }
        
        // OPTION 2: Store/Reset Position Method
        if (this.meshData && this.animations.pose.enabled) {
            this.meshData.forEach((meshInfo, index) => {
                ...
                // Temporarily move to origin for rotation
                ...
                meshInfo.rotationAccumulator += speed * direction;
            });
        }
        
        // Animate Spider-Man web effects
        if (this.spiderManGroup) {
            this.spiderManGroup.children.forEach(child => {
                if (child.userData.isWeb) {
                    child.rotation.y += 0.002;
                    child.position.y = Math.sin(time + child.userData.phase) * 5;
                }
            });
        }
        
        // Animate inner particles - swirling vortex effect
        if (this.innerParticles) {
            this.innerParticles.rotation.y += 0.003;
            const positions = this.innerParticles.geometry.attributes.position.array;
            for (let i = 0; i < positions.length; i += 3) {
                const originalRadius = Math.sqrt(positions[i] * positions[i] + positions[i + 2] * positions[i + 2]);
                const angle = Math.atan2(positions[i + 2], positions[i]) + 0.01;
                positions[i] = Math.cos(angle) * originalRadius;
                positions[i + 2] = Math.sin(angle) * originalRadius;
                positions[i + 1] += Math.sin(time * 2 + i * 0.1) * 0.5;
            }
            this.innerParticles.geometry.attributes.position.needsUpdate = true;
        }
        
        // Animate outer particles - floating effect
        if (this.outerParticles) {
            this.outerParticles.rotation.y -= 0.001;
            const positions = this.outerParticles.geometry.attributes.position.array;
            for (let i = 0; i < positions.length; i += 3) {
                positions[i + 1] += Math.sin(time + i * 0.05) * 0.2;
            }
    ...
    // Middle section removed - replaced with holographic compass
    createMiddleSection() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Initializing CSS2D Dashboard - CACHE BUSTED v2.0 - REFRESH NOW!');
    
    try {
        new HolographicCompassDashboard();
